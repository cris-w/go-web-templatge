...
func TestGetAppError(t *testing.T) {
	tests := []struct {
		...
		err       error
		wantCode  ErrorCode
		checkFunc func(*testing.T, *AppError)
	}{
		{
			name:     "已经是AppError",
			err:      ErrInvalidParam("测试错误"),
			wantCode: ErrCodeInvalidParam,
			checkFunc: func(t *testing.T, appErr *AppError) {
				assert.Equal(t, "测试错误", appErr.Message)
			},
		},
		{
			name:     "普通error转换",
			...
			checkFunc: func(t *testing.T, appErr *AppError) {
				assert.Equal(t, "服务器内部错误", appErr.Message)
